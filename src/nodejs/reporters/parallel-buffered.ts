...
const debug = Debugger('mocha:reporters:buffered');
import Base = require('../../reporters/base');
import Runnable = require('../../runnable');

type BufferedEvent = {eventName: string, data: object, error?: Error};
...
class ParallelBuffered extends Base {
   ...
   * @param {Runner} runner
   */
  constructor(runner: Runner, opts: Runner.RunnerOptions) {
    super(runner, opts);
  ...
  createListener(eventName: string) {
    const listener = (runnable: Runnable, err: Error) => {
      this.events.push(SerializableEvent.create(eventName, runnable, err));
    };
    return this.listeners.set(eventName, listener as any).get(eventName)!;
  }
   ...
   * @public
   */
  done(failures: number, callback: Function) {
