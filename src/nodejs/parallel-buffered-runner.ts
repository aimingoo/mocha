...
const debug = Debugger('mocha:parallel:parallel-buffered-runner');
import bufferedWorkerPool = require('./buffered-worker-pool');
const {BufferedWorkerPool} = bufferedWorkerPool;
const {setInterval, clearInterval} = global;
import Utils = require('../utils');
const {createMap, constants} = Utils;
const {MOCHA_ID_PROP_NAME} = constants;
import Errors = require('../errors');
const {createFatalError} = Errors;

const DEFAULT_WORKER_REPORTER = require.resolve(
  './reporters/parallel-buffered'
);

import type { CommandLineOptions } from '../cli/watch-run';
import Mocha = require('../mocha');
import Runnable = require('../runnable');
type MochaOptions = Mocha.MochaOptions;
type BufferedWorkerPool = bufferedWorkerPool.BufferedWorkerPool;
type RunOptions = {files?: string[], options?: CommandLineOptions & {[key: string]: any}};
...
const debugStats = (pool: BufferedWorkerPool) => {
...
type STATE_KEYS = keyof typeof STATE_KEYS;;
type States = {[key in STATE_KEYS]: Set<string>}
const states = createMap(STATE_KEYS) as States;
...
class ParallelBufferedRunner extends Runner {
  ...
  _linkedObjectMap: Map<string, any>;
  started: boolean = false;

  constructor(...args: ConstructorParameters<typeof Runner>) {
    super(...args);

    let state: STATE_KEYS = IDLE;
    Object.defineProperty(this, '_state', {
      get() {
        return state;
      },
      set(newState: STATE_KEYS) {
   ...
   * @private
   */
  _createFileRunner(pool: BufferedWorkerPool, options: MochaOptions) {
    /**
     ...
     */
    const emitEvent = (event: InternalEvent, failureCount: number) => {
     ...
     * @param {Object} event - Event having `eventName`, maybe `data` and maybe `error`
     */
    const linkEvent  = (event: InternalEvent) => {
      const stack: Array<{parent: InternalEvent, prop: string, obj?: any}> = [{parent: event, prop: 'data'}];
      while (stack.length) {
   ...
   * @private
   */
  _bindSigIntListener(pool: BufferedWorkerPool) {
   ...
   */
  run(callback: Runnable.Callback, {files = [], options = {}}: RunOptions = {}) {
    /**
    ...
    (async () => {
      ...
      try {
        ...
        // @ts-expect-error state machine updated maybe
        ...
        callback(this.failures as any);  // TODO: Recheck!
      } catch (err) {
  ...
  * }
  */
  linkPartialObjects(value: boolean) {
   ...
   * @public
   * @param {string} reporter - Absolute path to alternate reporter for worker processes to use
   * @returns {Runner}
   * @throws When in serial mode
   * @chainable
   */
  workerReporter(reporter: string) {
