...
const debug = Debugger('mocha:parallel:buffered-worker-pool');
...
type WorkerPoolOptions = Partial<typeof WORKER_POOL_DEFAULT_OPTS> & {
  env?: {[key: string]: string};
  maxWorkers?: number;
  onCreateWorker?: (opts: WorkerPoolOptions) => WorkerPoolOptions;
};
type WorkerPool = any;
/**
...
class BufferedWorkerPool {
  ...
  constructor(opts: WorkerPoolOptions = {}) {
    ...
    let counter = 0;
    const onCreateWorker = ({forkOpts}: WorkerPoolOptions) => {
   ...
   */
  async run(filepath: string, options: MochaOptions = {}) {
   ...
   * @private
   */
  static create(...args: ConstructorParameters<typeof BufferedWorkerPool>) {
...
namespace bufferedWorkerPool {
  export type BufferedWorkerPool = InstanceType<typeof BufferedWorkerPool>;
}

const bufferedWorkerPool = { BufferedWorkerPool };
export = bufferedWorkerPool;
