...
const debug = Debugger('mocha:serializer');

const SERIALIZABLE_RESULT_NAME = 'SerializableWorkerResult';
const SERIALIZABLE_TYPES = new Set(['object', 'array', 'function', 'error']);
type SerializedEvent = Pick<SerializableEvent, 'data'|'error'>; // {data?: object; error?: Error}
...
class SerializableWorkerResult {
  ...
  static deserialize(obj: any) {
    obj.events.forEach((event: SerializableEvent) => {
   ...
   */
  static isSerializedWorkerResult(value: any) {
    return (
...
class SerializableEvent {
   ...
   */
  constructor(eventName: string, originalValue?: any, originalError?: Error) {
    if (!eventName) {
   ...
   */
  static create(...args: ConstructorParameters<typeof SerializableEvent>) {
   ...
   */
  static _serialize(pairs: Array<object|string>, parent: {[key: string]: object}, key: string, seenObjects: WeakSet<object>) {
    let value = parent[key] as {[key: string]: any};
  ...
  serialize() {
    ...
    const result: SerializedEvent = Object.assign(Object.create(null), {
      data:
    ...
    const pairs: Array<[any, string]> = Object.keys(result).map(key => [result, key]);
   ...
   */
  static _deserializeError(value: any) {
    const error = new Error(value.message);
    error.stack = value.stack;
    Object.assign(error, value);
    delete (error as any).__type;
    return error;
  }
   ...
   */
  static _deserializeObject(parent: {[key: string|number]: any}, key: string|number) {
    if (key === '__proto__') {
      delete parent[key];
      return;
    }
   ...
   */
  static deserialize(obj: SerializedEvent) {
    if (!obj) {
...
namespace Serializer {
  export type SerializableWorkerResult = InstanceType<typeof SerializableWorkerResult>;
  export type SerializableEvent = InstanceType<typeof SerializableEvent>;
}
