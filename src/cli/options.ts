...
import runOptionMetadata = require('./run-option-metadata');
const {types, aliases} = runOptionMetadata;
import oneAndDones = require('./one-and-dones');
const {ONE_AND_DONE_ARGS} = oneAndDones;
const mocharc = require('../mocharc.json');
import runHelpers = require('./run-helpers');
const {list} = runHelpers;
import Config = require('./config');
const {loadConfig, findConfig} = Config;
const findUp = require('find-up');
import Debugger = require('debug');
const debug = Debugger('mocha:cli:options');
import NodeFlags = require('./node-flags');
const {isNodeFlag} = NodeFlags;
import Errors = require('../errors');
const {createUnparsableFileError} = Errors;
...
const coerceOpts = Object.assign(
  types.array.reduce(
    (acc, arg) =>
      Object.assign(acc, {
        [arg]: (v: any) => Array.from(new Set(globOptions.includes(arg) ? v : list(v)))
      }),
    {}
  ),
  types.boolean
    ...
    .reduce(
      (acc, arg) =>
        Object.assign(acc, {[arg]: (v: any) => (Array.isArray(v) ? v.pop() : v)}),
      {}
    )
);
 ...
 * @ignore
 */
const parse = (args: string|string[] = [], defaultValues = {}, ...configObjects: any[]) => {
 ...
 */
const loadRc = (args: ConfigOpts = {}): any => {
 ...
 */
const loadPkgRc = (args:ConfigOpts = {}): any => {
  let result;
 ...
 */
const loadOptions = (argv: string | string[] = []): any => {
  let args = parse(argv);
