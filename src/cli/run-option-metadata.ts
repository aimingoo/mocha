 ...
 * @private
 */
const TYPES: RunOptionMetadata["types"] = (exports.types = {
  array: [
    'extension',
    'file',
    'global',
...
const ALL_MOCHA_FLAGS = Object.keys(TYPES).reduce((acc, key) => {
  ...
  TYPES[key].forEach(flag => {
    acc.add(flag);
    const aliases: string[] = exports.aliases[flag] || [];
 ...
 * @private
 */
exports.isMochaFlag = (flag: string) => {
  return ALL_MOCHA_FLAGS.has(flag.replace(/^--?/, ''));
};

interface RunOptionMetadata {
  isMochaFlag(flag: string): boolean;
  aliases: {[key: string]: string[]};
  types: {[key: string]: string[]};
}

const runOptionMetadata = exports as RunOptionMetadata;
export = runOptionMetadata;