...
const debug = Debugger('mocha:cli:lookup-files');
 ...
 */
const isHiddenOnUnix = (pathname: string) => path.basename(pathname).startsWith('.');
 ...
 */
const hasMatchingExtname = (pathname: string, exts: string[] = []) =>
  exts
 ...
 */
const lookupFiles = function lookupFiles(
  filepath: string,
  extensions: string[] = [],
  ...
  fs.readdirSync(filepath).forEach((dirent: string) => {
    ...
    try {
      ...
      if (stat.isDirectory()) {
        if (recursive) {
          files.push(...castArray(lookupFiles(pathname, extensions, recursive)));
        }
        return;
      }
    } catch (ignored) {
