...
const {watchRun, watchParallelRun} = WatchRun;
import collectFiles = require('./collect-files');
...
import PluginLoader = require('../plugin-loader');

import type Mocha = require('../mocha');
import type Runner = require('../runner');
type PluginType = runHelpers.PluginType;
type PluginOptions = runHelpers.PluginOptions;
type PluginCache = runHelpers.PluginCache;
type requiresOptions = { ignoredPlugins?: string[] };
type CommandLineOptions = WatchRun.CommandLineOptions;
type FileCollectionOptions = WatchRun.FileCollectionOptions;
type chokidarWatcher = WatchRun.chokidarWatcher;
 ...
 * @private
 */
const exitMochaLater = (code: number) => {
  process.on('exit', () => {
...
const runHelpers = {} as RunHelpers;
 ...
 */
runHelpers.list = str =>
  Array.isArray(str) ? runHelpers.list(str.join(',')) : str.split(/ *, */);
 ...
 * @private
 */
runHelpers.handleRequires = async (requires = [], {ignoredPlugins = []} = {}) => {
 ...
 * @private
 */
const singleRun = async (mocha: Mocha, {exit}: CommandLineOptions, fileCollectParams: FileCollectionOptions) => {
 ...
 * @private
 */
const parallelRun = async (mocha: Mocha, options: CommandLineOptions, fileCollectParams: FileCollectionOptions) => {
  const files = collectFiles(fileCollectParams);
 ...
 * @returns {Promise<Runner>}
 */
runHelpers.runMocha = async (mocha, options) => {
  const {
    watch = false,
    extension = [],
    ignore = [],
 ...
 * @private
 */
runHelpers.validateLegacyPlugin = (opts, pluginType, map = {}) => {
  /**
  ...
  const createUnknownError = (err: any) =>
...
interface RunHelpers {
  ...
  handleRequires(requires?: string[], opts?: requiresOptions): Promise<object>;
  runMocha(mocha:Mocha, opts: CommandLineOptions): Promise<Runner|chokidarWatcher>;
  validateLegacyPlugin(opts: PluginOptions, pluginType: PluginType, map: PluginCache): void;
}

namespace runHelpers {
  export type PluginType = "reporter" | "ui";
  export type PluginOptions = {[key in PluginType]: string | string[]};
  export type PluginCache = {[key: string]: any}
}
