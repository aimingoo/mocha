...
const NodeFlags = {} as nodeFlagsSingleton;
 ...
 * @private
 */
NodeFlags.isNodeFlag = (flag, bareword = true) => {
  if (!bareword) {
 ...
 * @private
 */
NodeFlags.impliesNoTimeouts = flag => debugFlags.has(flag);
 ...
 * @private
 */
NodeFlags.unparseNodeFlags = opts => {
  var args: string[] = unparse(opts);
  return args.length
    ? args
        .join(' ')
        .split(/\b/)
...
interface nodeFlagsSingleton {
  isNodeFlag(flag: string, bareword?: boolean): boolean;
  impliesNoTimeouts(flag: string): boolean;
  unparseNodeFlags(opts: any): string[];
}
