...
const debug = Debugger('mocha:cli:watch');
...
var WatchRun = {} as WatchRunSingleton;
...
type BeforeWatchRun = (opts: BeforeWatchRunOptions) => Mocha | undefined;
type WatcherCreatorOptions = {
  watchFiles?: string[];
  watchIgnore?: string[];
  beforeRun?: BeforeWatchRun;
  fileCollectParams?: FileCollectionOptions;
}
type WatcherCreator = (mocha: Mocha, opts: WatcherCreatorOptions) => chokidarWatcher;
type Rerunner = {
  run(): void;
  scheduleRun(): void;
}
type RerunnerCreator = (mocha: Mocha, watcher: chokidarWatcher, opts?: WatcherCreatorOptions) => Rerunner;

type BeforeWatchRunOptions = {
  mocha: Mocha;
 ...
 * @private
 */
WatchRun.watchRun = (mocha, {watchFiles, watchIgnore}, fileCollectParams) => {
 ...
 * @private
 */
const createWatcher: WatcherCreator = (
  mocha: Mocha,
  ...
  let globalFixtureContext: Context.BaseContext;
 ...
 * @private
 */
const createRerunner: RerunnerCreator = (mocha: Mocha, watcher, {beforeRun} = {}) => {
 ...
 * @private
 */
const getWatchedFiles = (watcher: chokidarWatcher) => {
  type files = string[];
  const watchedDirs = watcher.getWatched() as {[key: string]: files};
 ...
 * @private
 */
const blastCache = (watcher: chokidarWatcher) => {
...
interface WatchRunSingleton {
  watchParallelRun(mocha: Mocha, opts: CommandLineOptions, fileCollectParams: FileCollectionOptions): Promise<chokidarWatcher>;
  watchRun(mocha: Mocha, opts: CommandLineOptions, fileCollectParams: FileCollectionOptions): chokidarWatcher;
};
...
namespace WatchRun {
  ...
  export type FileCollectionOptions = {
    extension?: string[]
  }
  export type chokidarWatcher = any;
}
